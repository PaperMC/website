...
export default async function JavadocsPage() {
  const [paperProject, foliaProject, velocityProject, waterfallProject] = await Promise.all([
  ...
  const velocityVersion = stringToSemver(Object.values(velocityProject.versions).flat()[0]); // Velocity's JDs don't have the -SNAPSHOT suffix (but we also can't use the version group)
  ...
  return (
    <>
      <header className="max-w-7xl flex flex-col items-center mx-auto px-4 pt-32 pb-16 lg:pt-48 lg:pb-26 gap-2">
        <h1 className="font-medium leading-normal lg:text-5xl lg:leading-normal text-4xl">Javadocs</h1>
        <p className="text-xl text-center mb-6">You can find the javadocs for our software below:</p>
        <div className="grid md:grid-cols-4 lg:mt-6 gap-2 px-2 xl:gap-4">
          ...
          <SoftwarePreview id="velocity" name="Velocity" javadocs={velocityVersion} />
          <SoftwarePreview id="waterfall" name="Waterfall" javadocs={waterfallVersion} />
        </div>
      </header>
    </>
  );
