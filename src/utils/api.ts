...
export async function fetchVersionBuilds(
  project: string,
  version: string,
...
export async function fetchVersionFamilyBuilds(
  project: string,
  family: string,
): Promise<VersionFamilyBuilds> {
  const response = await fetch(
    `${API_ENDPOINT}/projects/${project}/version_group/${family}/builds`,
  );
  return response.json();
}

export const generateVersionBuildDownloadURL = (
  project: string,
  version: string,
  build: number,
  file: string,
